              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '1.5rem' }}>
                ...
                {reports.filter(r => filterStatus === 'ALL' || r.status === filterStatus).map(report => (
                  <div key={report.id} className="glass-card" style={{ padding: '20px', display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ margin: '15px 0' }}>
                          ...
                          {report.original_audio && (
                            <div style={{ marginBottom: '10px' }}>
                              <audio src={report.original_audio.startsWith('http') ? report.original_audio : `http://127.0.0.1:8000${report.original_audio}`} controls style={{ width: '100%', height: '40px' }} />
                            </div>
                          )}
                    ...
                    {report.photo && <img src={report.photo.startsWith('http') ? report.photo : `http://127.0.0.1:8000${report.photo}`} alt="Report Image" style={{ width: '100%', maxHeight: '250px', objectFit: 'cover', borderRadius: '5px', marginBottom: '10px' }} />}
                  ...
                  {volunteers.filter(v => v.application_status === 'PENDING').map(v => (
                    <tr key={v.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td style={{ padding: '10px', color: 'orange' }}>
                        ...
                        {v.resume && <div><a href={v.resume.startsWith('http') ? v.resume : `http://127.0.0.1:8000${v.resume}`} target="_blank" rel="noreferrer" style={{ fontSize: '0.8rem', color: 'var(--secondary)' }}>📄 View Resume</a></div>}
                      </td>
                      <td style={{ padding: '10px' }}>
                    ...
                    {Object.values(requestsByReport).map((group: any) => (
                      <div key={group.report.id} className="glass-card" style={{ padding: '20px', borderLeft: '5px solid var(--primary)' }}>
                        <div style={{ marginBottom: '15px', paddingBottom: '15px', borderBottom: '1px solid var(--border)' }}>
                            ...
                            return (
                              <div style={{ margin: '15px 0' }}>
                                ...
                                {group.report.original_audio && (
                                  <div style={{ marginBottom: '10px' }}>
                                    <audio src={group.report.original_audio.startsWith('http') ? group.report.original_audio : `http://127.0.0.1:8000${group.report.original_audio}`} controls style={{ width: '100%', height: '40px' }} />
                                  </div>
                                )}
                          ...
                          {group.report.photo && <img src={group.report.photo.startsWith('http') ? group.report.photo : `http://127.0.0.1:8000${group.report.photo}`} alt="Report Image" style={{ width: '100%', maxHeight: '250px', objectFit: 'cover', borderRadius: '5px', marginBottom: '10px' }} />}
